...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/sqs_events/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/sqs_events/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/sqs_events/*"
            )
            ...
            #    "--include=src/saltext/sqs_events/*,tests/*"
            # )
        finally:
